...
Zet.declare('PostMessageGatewayStub', {
    ...
    defineBody : function(self){
        ...
        **/
        ...
        /** Set the HTML element where messages would be sent (a frame/parent) **/
        self.setElement = function setElement(element){
            self._element = element;
        };
...
Zet.declare('PostMessageGateway', {
    ...
    defineBody : function(self){
        ...
        **/
        ...
        self._transmitPostMessage = function _transmitPostMessage(node, msg, senderId, isHandshake){
            var postMsg, element;
            if (isHandshake) {
                element = parent;
            } else {
        ...
        self._receivePostMessage = function _receivePostMessage(event){
            ...
            if (self.isValidOrigin(event.origin)){
                ...
                // @TODO event.source - Check if the message senderId matches a stub.  Bind source as element if so
                ...
                if (Messaging.Message.isInstance(message) && 
                    ...
                    self.handleMessage(message, senderId);
                           ...
                           senderId in self._postNodes){
                    self._postNodes[senderId].setElement(event.source);
                }
            }
        };

        self._initiateHandshake = function _initiateHandshake(){
